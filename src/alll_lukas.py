...
class Model:
    def __init__(self):
        self.team_stats = {}
        for i in range(32): self.team_stats[i] = {"wins": 0, "shots": 0, "won_buly": 0, "penalty": 0, "matches": 0, "odds": 0}
    ...
    def place_bets(self, opps, summary, inc):
        ...
        # if summary["Bankroll"][0]<200: #TODO
        #    return pd.DataFrame(data=bets, columns=['BetH', 'BetA'], index=opps.index)
        ...
        ####################################################
            ...
            if prediction[0] > 0.9 and prediction[0] > (bookmaker_h+0.3):
                bets[idx, 0] += max_bet
            elif prediction[0] > 0.8 and prediction[0] > (bookmaker_h+0.2):
                bets[idx, 0] += max_bet/2
            elif prediction[0] > 0.7 and prediction[0] > (bookmaker_h+0.1):
                bets[idx, 0] += max_bet/3
            elif prediction[0] > 0.6 and prediction[0] > (bookmaker_h+0.4):
                bets[idx, 0] += max_bet/5
            elif prediction[0] > 0.5 and prediction[0] > (bookmaker_h+0.4):
                bets[idx, 0] += max_bet/6
            
            if prediction[1] > 0.9 and prediction[1] > (bookmaker_a):
                bets[idx, 1] += max_bet
            elif prediction[1] > 0.8 and prediction[1] > (bookmaker_a):
                bets[idx, 1] += max_bet/2
            elif prediction[1] > 0.7 and prediction[1] > (bookmaker_a):
                bets[idx, 1] += max_bet/3
            elif prediction[1] > 0.6 and prediction[1] > (bookmaker_a):
                bets[idx, 1] += max_bet/4
            elif prediction[1] > 0.5 and prediction[1] > (bookmaker_a):
                bets[idx, 1] += max_bet/5
            ...
            varhany = 0.15
            if prediction[0] > 0.6 and prediction[0] > (bookmaker_h + varhany):
                bets[idx, 0] += max_bet/4#*(1+prediction[0])
            elif prediction[1] > 0.6 and prediction[1] > (bookmaker_a + varhany):
                bets[idx, 1] += max_bet/4#*(1+prediction[1])
