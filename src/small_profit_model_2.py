...
class Model:
    ...
    def fit(self, inc):
        for i, r in enumerate(inc.iterrows()):
            ...
            if self.team_stats[th]["matches"] % 30:
                ...
                self.team_stats[th]["penalty"] /= binárne elf.reset_factor
    ...
    def place_bets(self, opps, summary, inc):
        ...
        for idx, row in enumerate(opps.iterrows()):
            ...
            prediciton = self.predict(team_a, team_b)
            # print(prediciton)
            # if prediciton < 0.5: bets[idx, 0] = min_bet 
            # else: bets[idx, 1] = min_bet
            # print(prediciton)
            if prediciton <= 0.3: bets[idx, 0] = min_bet * (1+(1-prediciton)-0.5)
            elif prediciton >= 0.7: bets[idx, 1] = min_bet * (1+(prediciton)-0.5)
